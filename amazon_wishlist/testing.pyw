...
def update():
    ...
    for item in db:
        ...
        pic=Label(master=wishlist, image=img)
        pic.photo=img
        pic.grid(row=row, column=0)
        Label(master=wishlist,text=var["name"]+"\t").grid(row=row,column=1)
        Label(master=wishlist,text= "|current:€"+str(var["price"])+"\t|").grid(row=row,column=2)
        Label(master=wishlist,text= "avg:€"+str(var["avg"])+"\t|").grid(row=row,column=3)
        Label(master=wishlist,text= "min:€"+str(var["lowest"])).grid(row=row,column=4)
