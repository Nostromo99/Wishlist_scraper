...
class book_depository_spider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        currency="€"
        return self.process(title,price,image,response,currency)

    def process(self,title,price,image,response,currency):
        db = shelve.open("list")

        price[0] = price[0].strip(currency)
        price[0] = price[0].replace(",", ".")
        info = Profile(name=title[0], price=float(price[0]), link=str(response)[5:-1], avg=float(price[0]),
                       lowest=float(price[0]), image_urls=image,
                       file=hashlib.sha1(image[0].encode("utf-8")).hexdigest())
        try:
